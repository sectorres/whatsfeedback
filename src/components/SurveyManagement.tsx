...
export function SurveyManagement() {
  ...
  const handleBarcodeSearch = async () => {
    ...
    try {
      ...
      const selectableIds = combined.filter(item => item.status === 'not_sent' || item.status === 'pending' || item.status === 'failed').map(item => item.campaign_send_id);
      setSelectedIds(prev => {
  ...
  const sendSelectedSurveys = async () => {
    ...
    const hasRespondedOrExpiredSurveys = items.some(item => selectedIds.has(item.campaign_send_id) && (item.status === 'responded' || item.status === 'expired' || item.rating != null));
    ...
    try {
      ...
      toast({
        ...
        description: data.surveys_sent > 0 ? `${data.surveys_sent} enviada${data.surveys_sent > 1 ? 's' : ''} com sucesso (${details.join(', ')})` : "Nenhuma pesquisa foi enviada"
      });
      loadSurveyStatus();
    } catch (error) {
      toast({
  ...
  const deleteSelectedSurveys = async () => {
    ...
    const surveysToDelete = items.filter(item => selectedIds.has(item.campaign_send_id) && item.status === 'not_sent');
    ...
    try {
      ...
      const {
        error
      } = await supabase.from('satisfaction_surveys').upsert(surveysToCancel, {
  ...
  const toggleAll = () => {
    ...
    const selectableItems = filteredItems.filter(item => item.status === 'not_sent' || item.status === 'pending' || item.status === 'failed');
  ...
  return <Card>
      <CardHeader>
        <div className="flex items-end justify-between gap-4">
          ...
          <div className="flex items-end gap-3">
            ...
            <Button onClick={sendSelectedSurveys} disabled={sending || selectedIds.size === 0} size="sm">
              {sending ? <>
      ...
      <CardContent>
        {loading ? <div className="flex items-center justify-center py-8">
          ...
          </div> : filteredItems.length === 0 ? <div className="text-center py-8 text-muted-foreground">
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  <TableHead className="w-12">
                    <Checkbox checked={filteredItems.filter(i => i.status === 'not_sent' || i.status === 'pending' || i.status === 'failed').length > 0 && selectedIds.size === filteredItems.filter(i => i.status === 'not_sent' || i.status === 'pending' || i.status === 'failed').length} onCheckedChange={toggleAll} />
                  </TableHead>
              ...
              <TableBody>
                {filteredItems.map(item => <TableRow key={item.campaign_send_id}>
                    <TableCell>
                       <Checkbox checked={selectedIds.has(item.campaign_send_id)} onCheckedChange={() => toggleSelection(item.campaign_send_id)} disabled={!(item.status === 'not_sent' || item.status === 'pending' || item.status === 'failed')} />
                    </TableCell>
                    ...
                    <TableCell className="text-sm text-muted-foreground">
                      {item.sent_at ? format(new Date(item.sent_at), "dd/MM/yyyy 'às' HH:mm", {
                  locale: ptBR
                }) : 'N/A'}
                    </TableCell>
                  </TableRow>)}
