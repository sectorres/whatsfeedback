...
export function RestrictedOrderPrefixesConfig() {
  ...
  const loadPrefixes = async () => {
    const { data } = await supabase
      .from("app_config")
      .select("config_value")
      .eq("config_key", "restricted_order_prefixes")
      .maybeSingle();
  ...
  const savePrefixes = async (prefixList: RestrictedPrefix[]) => {
    ...
    const { error } = await supabase
      ...
      .upsert(
        {
          config_key: "restricted_order_prefixes",
    ...
    if (error) {
      toast.error("Erro ao salvar prefixos inibidos");
      console.error(error);
    } else {
      toast.success("Prefixos inibidos atualizados");
    }
  };
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          ...
          Prefixos de Pedidos Inibidos (ABER)
        </CardTitle>
        <CardDescription>
          Pedidos que iniciam com estes prefixos serão ignorados no envio automático quando status ABER + Serie P
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="flex gap-2">
          <div className="flex-1">
            <Label htmlFor="prefix" className="sr-only">Prefixo do Pedido</Label>
            <Input
              id="prefix"
