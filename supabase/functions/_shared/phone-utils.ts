...
/**
 * Compara dois números de telefone ignorando formatação e diferenças de DDI/zeros à esquerda
 *
...
export function comparePhones(phone1: string, phone2: string): boolean {
  ...
  // Comparação exata pós-normalização
  if (clean1 === clean2) return true;

  // Comparação ignorando presença opcional do código do país
  const with55_1 = clean1.startsWith('55') ? clean1 : `55${clean1}`;
  const with55_2 = clean2.startsWith('55') ? clean2 : `55${clean2}`;

  return with55_1 === with55_2 || with55_1.endsWith(clean2) || with55_2.endsWith(clean1);
}
